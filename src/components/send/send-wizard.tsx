...
export function SendWizard() {
  ...
  const handleBulkSend = () => {
    if (bulkPreview.length < 2 || items.length === 0) return;
    ...
    for (const row of dataRows) {
      const contactValue = row[2]?.trim(); // contact column
      if (!contactValue) continue;
      const rowIsEmail = contactValue.includes("@");
      const rowChannel = rowIsEmail ? "email" : "sms";
      sendMessage({
        recipientContact: contactValue,
        contentBlocks: blocks,
        deliveryChannel: rowChannel as "email" | "sms",
      });
      count++;
    }
    ...
    setBulkSent(true);
    clear();
    toast.success(`Sent to ${count} recipients`);
  };
      ...
      <div className="mx-auto flex max-w-lg flex-col items-center gap-6 py-12 text-center">
        ...
        <div>
          ...
          <p className="mt-1 text-muted-foreground">
            Print or display this code for patients to scan.
          </p>
        </div>
        {/* Mock QR code */}
        <div className="rounded-lg border-2 border-dashed border-gray-300 p-6">
          ...
          <p className="mt-3 font-mono text-xs text-muted-foreground">
            peg.app/m/qr-demo-token
          </p>
        </div>
        <div className="flex gap-3">
          <Button variant="outline" onClick={() => toast.info("Download would trigger in production")}>
            Download PNG
          </Button>
          <Button variant="outline" onClick={() => toast.info("Print dialog would open in production")}>
            Print
          </Button>
        </div>
    ...
    return (
      <SuccessScreen
        ...
        subtitle={
          <>
            Materials sent to{" "}
            <span className="font-medium text-foreground">{bulkCount}</span>{" "}
            recipients. Delivery status will appear in Recipients.
          </>
        }
        onReset={resetAll}
      />
      ...
      <div className="mx-auto flex max-w-lg flex-col items-center gap-6 py-12 text-center">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Go to the Library to browse and select items, then come back here to
            send them.
          </p>
        </div>
        <Button
        ...
        <>
            ...
            <div className="mb-6">
              ...
              <div
                ...
                onDrop={(e) => {
                  ...
                  if (file && file.name.endsWith(".csv")) handleBulkUpload(file);
                }}
              >
                ...
                <p className="text-xs text-muted-foreground">
                  Required columns: first_name, last_name, contact
                </p>
                <input
                  type="file"
                  accept=".csv"
            ...
            <div className="mb-6">
              <div className="mb-3 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {bulkPreview.length - 1} recipient
                    {bulkPreview.length - 1 !== 1 ? "s" : ""} found
                  </p>
                </div>
                <Button
                  variant="ghost"
          ...
          <div className="flex items-center justify-between">
            ...
            <Button
              ...
              disabled={!bulkFile || bulkPreview.length < 2}
            >
              Send to {Math.max(0, bulkPreview.length - 1)} Recipients
            </Button>
          </div>
        </>
      )}
          ...
          <div className="mb-6 rounded-lg border bg-gray-50 p-4">
            ...
            <ul className="mt-2 space-y-1.5 text-sm text-muted-foreground">
              <li className="flex gap-2">
                <span className="shrink-0 text-purple-600">1.</span>
                A single shareable link is created for the selected content.
              </li>
              <li className="flex gap-2">
                <span className="shrink-0 text-purple-600">2.</span>
                A QR code is generated that points to this link.
              </li>
              <li className="flex gap-2">
                <span className="shrink-0 text-purple-600">3.</span>
                Download or print the QR code and display it in your office.
              </li>
              <li className="flex gap-2">
                <span className="shrink-0 text-purple-600">4.</span>
                Any patient can scan it to access the materials — no login needed.
              </li>
            </ul>
            <p className="mt-3 text-xs text-muted-foreground">
              Note: QR code sends track aggregate scan count only. Individual
              recipient tracking and reminders are not available.
            </p>
          </div>
          <div className="flex items-center justify-between">
            <Button
